"""
Value Lead (Damon Reeves) — Tier 2 Lane Lead.

Coordinates the Value lane: Health Monitor, Fathom Agent, QBR.
Reports to: Naveen Kapoor (cso_orchestrator)
Manages: health_monitor, fathom_agent, qbr_value
Traits: trend_analysis, customer_sentiment, delegation
"""
...
logger = logging.getLogger("agents.value_lead")
...
class ValueLead(BaseAgent):
    """Tier 2 Lane Lead for the Value lane."""
    ...
    def think(self, task: dict, context: dict) -> dict:
        ...
        print(f"[ValueLead | Damon Reeves] think() --calling Claude to choose specialists", flush=True)
        ...
        if "error" in parsed:
            ...
            print(f"[ValueLead | Damon Reeves] think() --Claude error, fallback plan: {fallback.get('specialists')}", flush=True)
            return fallback
        print(f"[ValueLead | Damon Reeves] think() --Claude chose: specialists={parsed.get('specialists')} rationale={str(parsed.get('rationale',''))[:80]}", flush=True)
        return parsed
    ...
    def act(self, task: dict, thinking: dict) -> dict:
        ...
        if not specialist_names:
            specialist_names = self._fallback_plan(task).get("specialists", ["health_monitor"])
        ...
        for spec_key in specialist_names:
            ...
            if not validation.get("passes", True) and validation.get("score", 1.0) < 0.6:
                logger.info(f"[value_lead] Requesting rework from {spec_key}: {validation.get('feedback', '')[:100]}")
            ...
            if isinstance(reflection, dict):
                ...
                if should_escalate or confidence < 0.4:
                    ...
                    logger.warning(f"[value_lead] Auto-escalation for {spec_key}: {escalation_reason}")
            ...
            logger.info(f"[value_lead] {spec_key} -> success={result.get('success')}")
    ...
    def finalize(self, task: dict, result: dict) -> dict:
        ...
        return {
            ...
            **parsed,
            "specialist_outputs": specialist_outputs,
            "reasoning_summary": parsed.get("summary", "Value lane deliverable ready."),
        }
    ...
    def _build_planning_prompt(self, task: dict, context: dict) -> str:
        ...
        parts.extend([
            ...
            "- health_monitor: Customer health score analysis (Dr. Aisha Okafor)",
            "- fathom: Call intelligence and meeting knowledge retrieval (Jordan Ellis)",
            "- meeting_followup: Meeting transcript analysis and email drafting (Riley Park)",
            "- qbr: Quarterly business review generation (Sofia Marquez)",
            "",
            "## Instructions",
            "Decide which specialists to engage and in what order.",
            "For EACH specialist you select, create a focused task brief:",
            'Return JSON: {"specialists": ["health_monitor"], "rationale": "...", "specialist_briefs": {"health_monitor": {"objective": "...", "focus_areas": ["..."], "expected_output": {"key": "format"}, "success_criteria": ["..."]}}}',
        ])
        return "\n".join(parts)

    def _build_synthesis_prompt(self, task: dict, outputs: dict) -> str:
        parts = [
            "## Value Lane Synthesis",
    ...
    def _fallback_plan(self, task: dict) -> dict:
        ...
        if "meeting" in event_type:
            return {"specialists": ["meeting_followup"], "rationale": "Meeting followup event"}
        if "call" in event_type or "fathom" in event_type or "zoom" in event_type:
            return {"specialists": ["fathom"], "rationale": "Call event"}
        if "health" in event_type:
            return {"specialists": ["health_monitor"], "rationale": "Health event"}
        if "qbr" in event_type or "renewal" in event_type:
            return {"specialists": ["qbr"], "rationale": "QBR/renewal event"}
        return {"specialists": ["health_monitor"], "rationale": "Default health check"}


AgentFactory.register("value_lead", ValueLead)
