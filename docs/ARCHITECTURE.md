...
- [x] Deal storage in database — `deals` table (1448 deals synced), Alembic migration `h8i9j0k1l2m3`
- [x] Customer-to-deal mapping — company name containment match (138 matched to customers)
- [x] Renewal date sync — `_populate_renewal_dates()` derives `customer.renewal_date` from deal `ced` (Contract End Date) property. Priority: ced > close_date+term > close_date+12m. 27/38 customers covered.
- [x] CS Manager sync — `_populate_cs_managers()` stores deal owner name in `customer.metadata_.cs_manager`. 18/38 customers covered.
- [x] Deal properties: `ced`, `deal_terms__months_` added to `DEAL_PROPERTIES` for renewal date derivation

**Note:** Deal `stage` column stores human-readable labels ("Closed Won", "Discovery Meeting"), NOT HubSpot internal IDs. All SQL filters must use labels.
...
### 9.1 Portfolio Brief (Wed & Fri 9 AM → #cs-executive-overview)

Generated by `executive_brief_service.py`. Six sections:

| Section            | Content                                                                      |
| ------------------ | ---------------------------------------------------------------------------- |
| Portfolio Health   | Risk distribution (healthy/watch/at-risk/critical), avg health, renewals in 60d with customer names |
| Attention Required | Top 5 critical items: health drops, stale P0/P1 tickets (age-based severity), open alerts. CS manager shown. |
| Ticket Summary     | 7-day velocity: created/resolved/net open, P0/P1 aging count               |
| Call Intelligence  | Call count, avg sentiment, negative calls, pending/overdue action items, key risks |
| Pipeline Snapshot  | Open deals (384), total value, 30d win rate, stalled deals, top deal        |
| Top 3 Actions      | AI-generated via Claude Haiku — prioritized, actionable, specific           |

**Manual trigger:** `POST /api/executive/brief`

### 9.2 Alert Rules (9 rules across 2 channels)

**Original rules → #cs-health-alerts (run with health monitor every 3 days):**

| Rule | Condition |
|------|-----------|
| health_drop_15 | Health score dropped >15 points in 7 days |
| critical_tickets_stale_p0 | P0 ticket open >7 days |
| critical_tickets_stale_p1 | P1 ticket open >10 days |
| renewal_at_risk | At-risk customer with renewal <60 days |
| negative_sentiment_streak | 3+ consecutive negative/mixed sentiment calls |

**Escalation rules → #cs-executive-urgent:**

| Rule | Condition | Schedule |
|------|-----------|----------|
| escalation_stale_p0 | P0 ticket open >3 days (configurable) | Daily 8:30 AM |
| escalation_stale_p1 | P1 ticket open >4 days (configurable) | Daily 8:30 AM |
| repeated_feature_requests | AI-grouped: 3+ customers request same feature in 30 days | Wed+Fri 9:15 AM |
| recurring_complaints | AI-grouped: 3+ tickets with same issue pattern in 30 days | Wed+Fri 9:15 AM |
| unanswered_action_items | Action item pending >3 days, no follow-up. Fire-once per item. | Wed+Fri 9:15 AM |

All thresholds configurable via env vars (`ESCALATION_STALE_P0_DAYS`, etc.).

### 9.3 Executive Reporting Checklist

- [x] Portfolio brief generator — `executive_brief_service.py`
- [x] Portfolio health calculator
- [x] Support velocity metrics
- [x] Sentiment trend tracker
- [x] Renewal date tracking (from HubSpot `ced` property)
- [x] CS Manager tracking (from deal owner)
- [ ] Feature demand ranker (partially covered by AI-grouped feature request alerts)
- [ ] Agent accuracy reporter
- [x] Wed+Fri 9 AM cron trigger for portfolio brief
- [x] Slack message to #cs-executive-overview
- [x] 5 original alert rules → #cs-health-alerts
- [x] 4 escalation rules → #cs-executive-urgent (stale P0/P1, features, complaints, actions)
- [x] Daily + Wed/Fri scheduling for escalation rules
- [x] Channel routing: `send_alert()` supports per-rule channel override
...
| 17  | ~~HubSpot integration: API connector, deal sync, stage change webhook~~ | ✅ Done — `hubspot_service.py`, 1448 deals synced, webhook live, daily 7 AM cron. Renewal dates from `ced` property (27/38 customers). CS manager from deal owners (18/38). |
| 18  | ~~Pre-Sales Funnel Agent: conversion rates, blocker analysis, stalled deals + deal win probability (Q3/Q4/Q11)~~ | ✅ Done — `presales_funnel_agent.py`, stage labels fixed (uses "Closed Won" not "closedwon"). Multi-factor probability model (5 weighted signals). Loss analysis cross-references Closed Lost deals with call recordings. |
...
| 20  | ~~Executive Reporter: weekly digest, portfolio brief, escalation rules~~ | ✅ Done — Portfolio brief (Wed+Fri 9 AM → #cs-executive-overview). 9 alert rules (5 original + 4 escalation). Escalation rules: stale P0/P1 daily, AI-grouped features/complaints + unanswered actions Wed+Fri. |
